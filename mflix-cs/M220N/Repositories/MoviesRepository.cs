...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        /// <param name="page">
        /// The page number, used in conjunction with <paramref name="moviesPerPage"/> to skip <see
        /// cref="Movie"/> documents for pagination.
        /// </param>
        /// <param name="sort">The field on which to sort the results.</param>
        /// <param name="sortDirection">
        /// The direction to use when sorting the <see cref="Movie"/>. 1 for ascending, -1 for descending
        /// </param>
        /// <param name="cancellationToken">Allows the UI to cancel an asynchronous request. Optional.</param>
        ...
        /// <returns>
        /// A <see cref="IReadOnlyList{T}"/> of <see cref="MovieByCountryProjection"/> objects
        /// </returns>
            ...
            )
        {
            return await _moviesCollection
               ...
               .Sort(Builders<Movie>.Sort.Descending(m => m.Title))
               .Project<MovieByCountryProjection>(Builders<Movie>.Projection
                                                                 .Include(m => m.Title))
               .ToListAsync(cancellationToken);
        }
            ...
            params string[] genres)
        {
            return await _moviesCollection
               ...
               .Limit(limit)
               .Skip(page * limit)
               .Sort(new BsonDocument(sortKey, DefaultSortOrder))
               .ToListAsync(cancellationToken);
        }
